...
import PatternInput from 'src/graph2/components/widgets/pattern/PatternInput';
  ...
  render() {
    ...
    if (viewport) {
      ...
      if (target) {
        ...
        const y = screenPos.y;
        const offsetX =
          -(this._ref.current.offsetWidth / 2) - parentClientRect.left;
        const offsetY =
          -(this._ref.current.offsetHeight / 2) - parentClientRect.top;
    ...
    return (
      <div
        ...
        />
        <div className={Style.tray_container}>{this.props.children}</div>
      </div>
    );
  }
}
