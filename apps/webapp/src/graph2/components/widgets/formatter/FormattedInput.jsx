...
class FormattedInput extends React.Component {
  ...
  focus() {
    this._debounceComponent.current.focus();
  }
  blur(submit = true) {
  ...
  setValue(value, updatePrevious = false) {
    this.setState((prev, props) => {
      ...
      if (component.hasCaretSelection()) {
        ...
        result =
          result.substring(0, caretPosition.start) +
          value +
          result.substring(caretPosition.end);
        component.setCaretPosition(
          caretPosition.start,
          caretPosition.start + value.length
        );
      } else {
        result = value;
  ...
  appendValue(value, updatePrevious = false) {
    this.setState((prev, props) => {
      ...
      if (component.hasCaretPosition()) {
        const caretPosition = component.getCaretPosition();
        if (
          caretPosition.end + 1 >= previous.length ||
          caretPosition.start > caretPosition.end
        ) {
          result = previous + value;
        } else {
          result =
            previous.substring(0, caretPosition.end) +
            value +
            previous.substring(caretPosition.end);
        }

        component.setCaretPosition(
          caretPosition.start,
          caretPosition.end + value.length
        );
      } else {
        result = previous + value;
      }
  ...
  getDebounceComponent() {
    return this._debounceComponent.current;
  }
  getInputElement() {
    return this._debounceComponent.current.getInputElement();
  }
  getValue() {
  ...
  hasFocus() {
    return this._debounceComponent.current.hasFocus();
  }
