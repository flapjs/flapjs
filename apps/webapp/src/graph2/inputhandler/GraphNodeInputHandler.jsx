...
export const GRAPH_EVENT_NODE_EDIT_WHILE_DELETE =
  'error-node-edit-while-delete';
...
class GraphNodeInputHandler extends AbstractInputHandler {
  ...
  onInputEvent(pointer) {
    ...
    if (inputController.isTrashMode()) {
      if (currentTargetType === EVENT_SOURCE_NODE) {
        ...
        graphController.emitGraphEvent(GRAPH_EVENT_NODE_DELETE, {
    ...
    if (
      ...
      graphController.openLabelEditor(
        currentTargetSource,
        currentTargetSource.getNodeLabel(),
        (target, value, hasChanged) => {
          if (SHOULD_DELETE_NODE_WITH_EMPTY_LABEL && !value) {
            ...
            graphController.emitGraphEvent(GRAPH_EVENT_NODE_LABEL_CHANGE, {
              target: currentTargetSource,
            });
          }
        }
  ...
  onDragMove(pointer) {
    ...
    if (selectionBox && selectionBox.hasSelection()) {
      ...
      this.moveNodeTo(
        this._graphController.getGraph(),
        targetSource,
        pointer.x,
        pointer.y
      );
  ...
  onDragStop(pointer) {
    ...
    if (inputController.isTrashMode()) {
      if (selectionBox.hasSelection()) {
        ...
        selectionBox.clearSelection();
        graphController.emitGraphEvent(GRAPH_EVENT_NODE_DELETE_ALL, {
          target: dst,
        });
      } else {
    ...
    if (selectionBox.hasSelection()) {
      graphController.emitGraphEvent(GRAPH_EVENT_NODE_MOVE_ALL, {
        target: selectionBox.getSelection(),
      });
    } else {
      graphController.emitGraphEvent(GRAPH_EVENT_NODE_MOVE, {
        target: targetSource,
      });
    }
  }
