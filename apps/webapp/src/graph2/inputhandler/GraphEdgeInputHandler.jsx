...
export const GRAPH_EVENT_EDGE_EDIT_WHILE_DELETE =
  'error-edge-edit-while-delete';
...
class GraphEdgeInputHandler extends AbstractInputHandler {
  ...
  onInputEvent(pointer) {
    ...
    if (inputController.isTrashMode()) {
      if (
        currentTargetType === EVENT_SOURCE_EDGE ||
        ...
        graphController.emitGraphEvent(GRAPH_EVENT_EDGE_DELETE, {
    ...
    if (
      ...
      const currentTargetSource = inputController.getCurrentTargetSource();
      const graphController = this._graphController;
      graphController.openLabelEditor(
        currentTargetSource,
        currentTargetSource.getEdgeLabel(),
        (target, value, hasChanged) => {
          if (this._shouldDeleteEdgeWithEmptyLabel && !value) {
            ...
            graphController.emitGraphEvent(GRAPH_EVENT_EDGE_LABEL_CHANGE, {
              target: currentTargetSource,
            });
          }
        }
  ...
  onDragStart(pointer) {
    ...
    const currentTargetType = inputController.getCurrentTargetType();
    if (
      !inputController.isMoveMode(pointer.getInputAdapter()) &&
      currentTargetType === EVENT_SOURCE_NODE
    ) {
      const graphController = this._graphController;
      ...
      const targetEdge = graph.createEdge(
        currentTargetSource,
        this._cachedPointer
      );

      inputController.bindActiveTarget(
        targetEdge,
        EVENT_SOURCE_FORWARD_ENDPOINT,
        false
      );
  ...
  onDragMove(pointer) {
    ...
    if (targetType === EVENT_SOURCE_EDGE) {
      ...
      this.handleDragMoveForEndpoint(
        targetSource,
        targetType,
        pointer,
        inputController
  ...
  handleDragMoveForEndpoint(
    ...
    if (immediateTargetType === EVENT_SOURCE_NODE) {
      ...
      this._cachedPointer.x = lerp(
        this._cachedPointer.x,
        pointer.x,
        EDGE_POSITION_INTERPOLATION_DELTA
      );
      this._cachedPointer.y = lerp(
        this._cachedPointer.y,
        pointer.y,
        EDGE_POSITION_INTERPOLATION_DELTA
      );
  ...
  onDragStop(pointer) {
    ...
    if (targetType === EVENT_SOURCE_EDGE) {
      graphController.emitGraphEvent(GRAPH_EVENT_EDGE_QUAD_CHANGE, {
        target: targetSource,
      });
    } else {
      ...
      if (!edgeTo || edgeTo === this._cachedPointer) {
        if (this._shouldDeleteEdgePlaceholder) {
          ...
          if (isNewEdge) {
            ...
            graphController.emitGraphEvent(GRAPH_EVENT_EDGE_CREATE, {
              target: targetSource,
            });
          } else {
            graphController.emitGraphEvent(GRAPH_EVENT_EDGE_TO_CHANGE, {
              target: targetSource,
            });
          }
        }
        ...
        graphController.openLabelEditor(
          targetSource,
          isNewEdge ? null : targetSource.getEdgeLabel(),
          (target, value, hasChanged) => {
            if (this._shouldDeleteEdgeWithEmptyLabel && !value) {
              graphController.getGraph().deleteEdge(targetSource);
              graphController.emitGraphEvent(GRAPH_EVENT_EDGE_DELETE, {
