  ...
  onDragMove(pointer) {
    ...
    if (targetType === EVENT_SOURCE_START_MARKER) {
      ...
      if (immediateTargetType === EVENT_SOURCE_NODE) {
        const immediateTargetSource =
          inputController.getImmediateTargetSource();
        ...
        this._cachedPointer.x = lerp(
          ...
          pointer.x,
          START_MARKER_POSITION_INTERPOLATION_DELTA
        );
  ...
  onDragStop(pointer) {
    ...
    if (targetType === EVENT_SOURCE_START_MARKER) {
      if (!this._ghostMarker || this._ghostMarker === this._cachedPointer) {
        ...
        this._graphController.emitGraphEvent(GRAPH_EVENT_START_MARKER_CHANGE, {
          target: this._ghostMarker,
        });
      }
