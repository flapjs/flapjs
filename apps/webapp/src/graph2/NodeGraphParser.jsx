...
class NodeGraphParser extends Parser {
  ...
  parse(data, dst = null) {
    ...
    if (!dataVersion.canSupportVersion(VERSION)) {
      throw new Error(
        "Unable to parse data for incompatible version '" + dataVersion + "'"
      );
    }
  ...
  compose(target, dst = null) {
    ...
    for (let i = 0; i < edgeCount; ++i) {
      ...
      if (edge) {
        ...
        const sourceIndex = nodeIndices.has(edgeSource)
          ? nodeIndices.get(edgeSource)
          : -1;
        const destinationIndex = nodeIndices.has(edgeDestination)
          ? nodeIndices.get(edgeDestination)
          : -1;
        const edgeData = {
          id: elementID,
          from: sourceIndex,
  ...
  onParseGraphCreate(graphData, targetGraph) {
    ...
    if (!targetGraph) {
      targetGraph = new NodeGraph(
        GraphNode,
        hasQuad ? QuadraticEdge : GraphEdge
      );
    } else {
      targetGraph.clear();
    }
