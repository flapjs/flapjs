...
export const DefaultEndpointRenderer = (props) => (
  <React.Fragment>
    ...
    />
    <circle
      cx={props.position.x - props.length * Math.cos(-props.angle + HALF_PI)}
      cy={props.position.y - props.length * Math.sin(-props.angle + HALF_PI)}
      r={props.length}
      fill="transparent"
      cursor="pointer"
...
class EdgeRenderer extends React.Component {
  ...
  render() {
    ...
    if (Math.abs(ftrad - fcrad) <= QUAD_RADIAN_THRESHOLD) {
      toAngle =
        Math.atan2(centerPoint.x - toPoint.x, centerPoint.y - toPoint.y) +
        Math.PI;
      fromAngle =
        Math.atan2(centerPoint.x - fromPoint.x, centerPoint.y - fromPoint.y) +
        Math.PI;
