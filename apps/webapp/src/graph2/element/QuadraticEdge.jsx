...
import { getDirectionalVector, getMidPoint } from 'src/util/MathHelper';
...
class QuadraticEdge extends GraphEdge {
  ...
  setEdgeTo(node) {
    ...
    if (node !== prevNode) {
      ...
      if (!node) {
        ...
        this._quad.length =
          this._from.getNodeSize() + this.getPlaceholderLength();
      }
      return result;
    }
  ...
  getQuadraticLength() {
    return this._quad.length;
  }
  getQuadraticRadians() {
    return this._quad.radians;
  }
  getQuadratic() {
    return this._quad;
  }
  ...
  getCenterPoint(dst = { x: 0, y: 0 }) {
    ...
    if (!to) {
      ...
      const placeholderLength =
        from.getNodeSize() + this.getPlaceholderLength() / 2;
  ...
  getEndPoint(dst = { x: 0, y: 0 }) {
    ...
    if (!to) {
      ...
      const py = Math.sin(radians);
      const placeholderLength =
        from.getNodeSize() + this.getPlaceholderLength();
      dst.x = from.x + px * placeholderLength;
      dst.y = from.y + py * placeholderLength;
      return dst;
    }
  ...
  getHashString(usePosition = true) {
    if (usePosition) {
      return (
        super.getHashString(usePosition) +
        ':' +
        this._quad.radians +
        ',' +
        this._quad.length
