...
import Style from './DrawerView.module.css';
...
class DrawerView extends React.Component {
  ...
  onDrawerHandleMove(e) {
    ...
    if (
      !isDrawerSideBottom &&
      Math.abs(result - DRAWER_HANDLE_CUSTOM_SNAP_WIDTH) <
        DRAWER_HANDLE_MIN_SIZE_BUFFER
    ) {
  ...
  render() {
    ...
    const isDrawerSideBottom = drawerSide === DRAWER_SIDE_BOTTOM;
    const shouldDrawerBarSideways =
      !isDrawerSideBottom &&
      (!isDrawerOpen || drawerDirection === DRAWER_BAR_DIRECTION_VERTICAL);
    const showDrawerHandle = isDrawerOpen || this._handlingGrab;
    const shouldDrawerOpenFull = this._isfull;
    const shouldHideDrawerContent =
      (DRAWER_SHOULD_HIDE_CONTENT_ON_RESIZE &&
        (this._handlingGrab || this._handlingResize)) ||
      !isDrawerOpen;
    ...
    return (
      <div
        ...
        <div
          ref={(ref) => (this.drawerElement = ref)}
          className={
            Style.app_drawer +
            (isDrawerOpen ? ' open ' : '') +
          ...
          <div
            className={Style.drawer_handle + (showDrawerHandle ? ' show ' : '')}
            onMouseDown={this.onDrawerHandleGrab}>
            <span>{'||'}</span>
          </div>
          <div className={Style.drawer_content}>
            <nav className={Style.drawer_content_bar}>
              <IconButton
                className={Style.drawer_tab_expander}
                onClick={this.onDrawerExpand}>
                <ExpandDownIcon />
              </IconButton>
              {showDrawerTabs &&
...
function getCSSDrawerWidth(drawerElement) {
  return parseInt(
    window
      ...
      .getPropertyValue(DRAWER_WIDTH_CSSVAR)
      .replace(/\D/g, '')
  );
}
