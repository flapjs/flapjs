import AbstractAutoSaveHandler from 'src/util/storage/AbstractAutoSaveHandler';
...
class AppSaver extends AbstractAutoSaveHandler {
  ...
  onAutoSaveUpdate(dataStorage) {
    ...
    try {
      app
        .getExportManager()
        .tryExportTargetToData('session', session)
        .then((result) => {
          dataStorage.setDataAsObject(
            'graph-' + currentModuleName,
            result.data
          );
        });
    } catch (e) {
