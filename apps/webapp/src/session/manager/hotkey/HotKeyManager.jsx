...
class HotKeyManager {
  ...
  registerHotKey(name, keys, callback) {
    ...
    if (keys.length != 1)
      throw new Error('Multiple key operations not supported');
  ...
  onKeyDown(e) {
    ...
    if (!e.repeat) {
      ...
      if (!result && matchesHotKeyEvent(e, this._altHotkey))
        result = this._altHotkey;
  ...
  captureKeyEvent(e, isKeyUp = false) {
    ...
    return (
      isKeyUp &&
      !(
        this._cachedEvent.ctrlKey ||
        this._cachedEvent.metaKey ||
        this._cachedEvent.altKey ||
        this._cachedEvent.shiftKey
      )
    );
  }
}
