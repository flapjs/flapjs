...
import Style from './HotKeyView.module.css';
...
class HotKeyView extends React.Component {
  ...
  render() {
    ...
    return (
      <div
        id={this.props.id}
        className={Style.hotkey_container + ' ' + this.props.className}
        ...
        {showHotKeys &&
          this._cachedHotKeys.map((e, i) => {
            ...
            return (
              <div
                key={
                  e.localizedKeys + ':' + i + ':' + (isActive ? useCount : 0)
                }
                className={Style.hotkey + (isActive ? ' active ' : '')}>
                <label className={Style.hotkey_label}>{e.localizedKeys}</label>
