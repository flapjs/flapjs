import { getFileExtFromName } from '../FileHelper';
...
class ImportManager {
  ...
  tryImportFile(fileBlob) {
    ...
    if (!(fileBlob instanceof File))
      throw new Error('Unable to import object as file');

    return this._textUploader
      .uploadFile(fileBlob)
      .then((result) => {
  ...
  tryImportFileFromData(fileName, fileType, fileData) {
    ...
    if (importers && importers.length > 0) {
      return importers.reduce(
        (acc, value) =>
          acc.catch((err) =>
            value.src.importFileData(fileName, value.type, fileData)
          ),
        Promise.reject()
      );
    } else {
