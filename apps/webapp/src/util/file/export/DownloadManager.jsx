...
import SVGImageDownloader from './SVGImageDownloader';
...
class DownloadManager {
  ...
  tryDownloadFile(fileName, downloadType, fileData, opts) {
    ...
    if (downloaders && downloaders.length > 0) {
      return downloaders.reduce(
        (acc, value) =>
          acc.catch((e) =>
            value.downloadFile(fileName, downloadType, fileData, opts)
          ),
        Promise.reject()
      );
    } else {
   ...
   */
  isEmpty() {
    return this._downloaders.size <= 0;
  }
}
