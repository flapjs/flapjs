...
class InputAdapter extends InputContext {
  ...
  bindContext(context) {
    if (!(context instanceof InputContext))
      throw new Error(
        'Cannot bind invalid context - must be an instance of InputContext'
      );
  ...
  initialize(element) {
    if (!(element instanceof SVGElement))
      throw new Error("Missing SVG element for input adapter's viewport");
    if (this._element)
      throw new Error(
        'Trying to initialize an InputAdapter already initialized'
      );

    this._viewportAdapter.setElement((this._element = element));
    this._pointer = new InputPointer(
      this,
      this._viewportAdapter,
      this._element
    );
  ...
  destroy() {
    if (!this._element)
      throw new Error(
        'Trying to destroy an InputAdapter that is not yet initialized'
      );
  ...
  onMouseMove(e) {
    const mouse = this._viewportAdapter.transformScreenToView(
      e.clientX,
      e.clientY
    );
    ...
    if (!this._dragging) {
      if (pointer.getDistanceSquToInitial() > this._draggingRadiusSqu) {
        ...
        const result = this.handleEvent(
          'onDragStart',
          pointer,
          inputEventPosition.x,
          inputEventPosition.y
        );
        if (!result) {
          this._dragging = false;
