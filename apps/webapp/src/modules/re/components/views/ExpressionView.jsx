...
class ExpressionView extends React.Component {
  ...
  onClick(e) {
    ...
    const scope = machineController._parser.scopeFromSpaceIndexing(
  ...
  _appendSymbol(machineController, symbol) {
    ...
    const cursorPos = this._inputElement.selectionStart;
    const newExpression =
      currentExpression.slice(0, cursorPos) +
      symbol +
  ...
  render() {
    ...
    const readableValue = machineController
      .getMachineExpression()
      .replace(new RegExp(UNION, 'g'), UNION_CHAR);
      ...
      <div
        ...
        <div
          className={
            Style.view_widget +
            ' ' +
            ExpressionViewStyle.expression_tray +
            ' ' +
            ExpressionViewStyle.tray_important
          }>
          <button
            title="Epsilon"
            onClick={() => {
              this._appendSymbol(machineController, EMPTY);
            }}>
            {EMPTY}
          </button>
          <button
            title="Union"
            onClick={() => {
              this._appendSymbol(machineController, UNION);
            }}>
            {UNION_CHAR}
          </button>
          <button
            title="Concat"
            onClick={() => {
              this._appendSymbol(machineController, CONCAT);
            }}>
            {CONCAT}
          </button>
          <button
            title="Kleene Star"
            onClick={() => {
              this._appendSymbol(machineController, KLEENE);
            }}>
            {KLEENE}
          </button>
          <button
            title="Kleene Plus"
            onClick={() => {
              this._appendSymbol(machineController, PLUS);
            }}>
            {PLUS}
          </button>
          <button
            title="Sigma"
            onClick={() => {
              this._appendSymbol(machineController, SIGMA);
            }}>
            {SIGMA}
          </button>
          <button
            title="Empty Set"
            onClick={() => {
              this._appendSymbol(machineController, EMPTY_SET);
            }}>
            {EMPTY_SET}
          </button>
        </div>
        ...
        <div
          className={
            ...
            ExpressionViewStyle.expression_tray +
            ' ' +
            ExpressionViewStyle.tray_symbol
          }>
          {terminals.map((e) => {
            return (
              <button
                key={e}
                onClick={() => {
                  this._appendSymbol(machineController, e);
                }}>
                {e}
              </button>
            );
