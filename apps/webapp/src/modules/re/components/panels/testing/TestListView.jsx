...
import { downloadText } from 'src/util/Downloader';

import IconButton from 'src/components/IconButton';
import IconUploadButton from 'src/components/IconUploadButton';
...
import { solveFSA } from 'src/modules/fsa2/machine/FSAUtils';
...
class TestListView extends React.Component {
  ...
  /** @override */
  componentDidMount() {
    this.props.machineController
      .getExpressionChangeHandler()
      .addListener(this.onExpressionChange);
  }
  ...
  componentWillUnmount() {
    this.props.machineController
      .getExpressionChangeHandler()
      .removeListener(this.onExpressionChange);
  }
  ...
  render() {
    ...
    return (
      <div
        ...
        <div className={Style.test_list_container + (empty ? ' empty ' : '')}>
          <IconButton
            className={Style.test_list_add}
            title={'Add'}
          ...
          <IconButton
            className={Style.test_list_runall}
            title={'Run All'}
            onClick={this.onTestRunAll}>
            <RunningManIcon />
          </IconButton>
        </div>
      </div>
