import { stringHash } from 'src/util/MathHelper';

import RE from './machine/RE';
import REParser from './machine/REParser';
import { convertToNFA } from './machine/REUtils';

import ExpressionChangeHandler from './ExpressionChangeHandler';
...
class MachineController {
  constructor() {
    ...
    this._expressionChangeHandler = new ExpressionChangeHandler(
      EXPRESSION_REFRESH_RATE
    );
  ...
  renameSymbol(symbol, nextSymbol) {
    ...
    const nextExpression = prevExpression.replace(
      new RegExp(symbol, 'g'),
      nextSymbol
    );
  ...
  getEquivalentFSA() {
    if (
      !this._equalFSA ||
      stringHash(this._machine.getExpression()) !== this._equalREHash
    ) {
      this._equalREHash = stringHash(this._machine.getExpression());
