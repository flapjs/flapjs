...
function formatEdge(graph, edge) {
  ...
  for (const otherEdge of graph.getEdges()) {
    ...
    if (
      otherEdge.getEdgeFrom() === edgeSource &&
      otherEdge.getEdgeTo() === edgeDestination
    ) {
  ...
  if (!edge.isSelfLoop()) {
    ...
    for (const otherEdge of graph.getEdges()) {
      if (
        otherEdge.isQuadratic() &&
        Math.abs(otherEdge.getQuadratic().length) >= parallelEdgeHeight * 2
      )
        continue;
      if (
        otherEdge.getEdgeTo() === edgeSource &&
        otherEdge.getEdgeFrom() === edgeDestination
      ) {
        edge.setQuadraticRadians(HALFPI).setQuadraticLength(parallelEdgeHeight);
        otherEdge
          .setQuadraticRadians(HALFPI)
          .setQuadraticLength(parallelEdgeHeight);
        flag = true;
    ...
    if (!flag) {
      const maxNodeSize = Math.max(
        edgeSource.getNodeSize(),
        edgeDestination.getNodeSize()
      );
