import BroadcastHandler from 'src/session/manager/broadcast/BroadcastHandler';

import { JSON as JSONGraphParser } from 'src/modules/fsa2/FSAGraphParser';
import FSA from 'src/modules/fsa2/machine/FSA';
import { isEquivalentFSA } from 'src/modules/fsa2/machine/FSAUtils';
...
class FSABroadcastHandler extends BroadcastHandler {
  ...
  testEquivalence(targetSessionID) {
    this._broadcastManager.sendMessageTo(
      targetSessionID,
      MACHINE_REQUEST_MESSAGE_TYPE
    );
  }
  ...
  onBroadcastMessage(type, src, dst, message) {
    if (type === MACHINE_REQUEST_MESSAGE_TYPE) {
      const graph = this._broadcastManager
        .getApp()
        .getSession()
        .getCurrentModule()
        .getGraphController()
      ...
      const machineController = this._broadcastManager
        .getApp()
        .getSession()
        .getCurrentModule()
      ...
      window.alert(
        'The machines are ' + (result.value ? 'EQUAL' : 'NOT EQUAL') + '!'
      );
      return true;
    }
