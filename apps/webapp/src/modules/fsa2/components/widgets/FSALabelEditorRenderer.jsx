...
import {
  SYMBOL_SEPARATOR,
  EMPTY_CHAR,
} from 'src/modules/fsa2/graph/element/FSAEdge';
...
class FSALabelEditorRenderer extends React.Component {
  ...
  render() {
    ...
    const showEdgeTray =
      labelEditor && labelEditor.getTarget() instanceof GraphEdge;
    const showNodeTray =
      labelEditor && labelEditor.getTarget() instanceof GraphNode;
        ...
        {showEdgeTray && (
          <span>
            ...
            <span className={Style.tray_default}>
              ...
              {showDefault && DEFAULT_SYMBOLS.map((e) => this.renderSymbol(e))}
            </span>
          </span>
        )}
        {showNodeTray && (
          <span>
            <PatternInputButton
              parent={labelEditor.inputComponent}
              title={
                labelEditor.getTarget().getNodeCustom() ? 'Custom' : 'Auto'
              }
              onClick={() => {
                labelEditor
                  .getTarget()
                  .setNodeCustom(!labelEditor.getTarget().getNodeCustom());
              }}
            />
            <PatternInputButton
              parent={labelEditor.inputComponent}
              title={
                labelEditor.getTarget().getNodeAccept() ? 'Accept' : 'Reject'
              }
              onClick={() => {
                labelEditor
                  .getTarget()
                  .setNodeAccept(!labelEditor.getTarget().getNodeAccept());
              }}
            />
          </span>
        )}
