...
import Style from './TransitionTableView.module.css';

import { EMPTY_CHAR } from 'src/modules/fsa2/graph/element/FSAEdge';
import { EMPTY_SYMBOL } from 'src/modules/fsa2/machine/FSA';
...
class TransitionTableView extends React.Component {
      ...
      switch (rowAxis) {
        case SYMBOL_AXIS:
          for (const symbol of machine.getAlphabet()) {
            rowComponents.push(
              this.renderTableEntryForSymbolAxis(machine, state, symbol)
            );
          }
          rowComponents.push(
            this.renderTableEntryForSymbolAxis(machine, state, EMPTY_SYMBOL)
          );
          break;
        case STATE_AXIS:
      ...
      case SYMBOL_AXIS:
        for (const symbol of machine.getAlphabet()) {
          result.push(
            <th
              key={symbol}
              scope="col"
              className={Style.table_axis_header + ' col'}>
              {symbol}
            </th>
          );
        }
        result.push(
          <th
            ...
            scope="col"
            className={
              Style.table_axis_header + (disabled ? ' disabled ' : '') + ' col'
            }>
            {EMPTY_CHAR}
          </th>
        );
  ...
  render() {
    ...
    return (
      <div
        id={this.props.id}
        className={Style.table_container + ' ' + this.props.className}
