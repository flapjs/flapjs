...
class TransitionChartView extends React.Component {
  ...
  renderTransitionEntry(machine, state, symbol) {
    ...
    return (
      <tr key={state.getStateID() + ':' + symbol}>
        <td className={Style.chart_key + (error ? ' error ' : '')}>
          {'(' + state.getStateLabel() + ',' + symbol + ')'}
        </td>
        <td className={Style.chart_value + (error ? ' error ' : '')}>
          {transitionString}
        </td>
      </tr>
    );
  ...
  render() {
    ...
    return (
      <div
        id={this.props.id}
        className={Style.chart_container + ' ' + this.props.className}
