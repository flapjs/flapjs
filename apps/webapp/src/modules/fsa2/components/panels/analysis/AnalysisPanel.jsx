...
import { isEquivalentFSAWithWitness } from 'src/modules/fsa2/machine/util/EqualFSA';
import {
  ...
  onEquivalentTest(e) {
    createMachineFromFileBlob(e)
      .then((result) => {
        ...
        const equivalenceResult = isEquivalentFSAWithWitness(
          result,
          currentMachine
        );
        if (equivalenceResult.value) {
          ...
          if (!equivalenceResult.witnessString) {
            this.setState({
              isEqual: false,
              witnessString: 'Sorry, the machines have different alphabets',
            });
          } else {
            this.setState({
              isEqual: false,
              witnessString: 'Witness: ' + equivalenceResult.witnessString,
            });
          }
        }
      })
