...
export function intersectDFA(dfa1, dfa2, dst = new FSA(true)) {
  ...
  while (remainingPairIDs.length > 0) {
    ...
    for (const symbol of alphabet) {
      ...
      if (result.length !== 1) {
        throw new Error(
          "Found non-deterministic transition from '" +
            parent1.getStateLabel() +
            "' for '" +
            symbol +
            "' - transitions to '" +
            result.join(',') +
  ...
  for (const [statePairID, statePair] of pairIDs.entries()) {
    const state = dst.createState(
      '(' +
        statePair[0].getStateLabel() +
        ',' +
        statePair[1].getStateLabel() +
        ')'
    );
...
function checkSameAlphabet(fsa1, fsa2) {
  ...
  for (let symbol of fsa1.getAlphabet()) {
    alphabet.add(symbol);
  }
