...
import * as ColorHelper from 'src/util/ColorHelper';
...
class NodalGraphImageExporter extends SessionExporter {
  ...
  exportTarget(exportType, target) {
    ...
    const svg = this.processSVGForExport(
      svgElement,
      width,
      height,
      currentModule
    ...
    const fileExt =
      this._imageType === IMAGE_TYPE_BW ? IMAGE_TYPE_PNG : this._imageType;
  ...
  processSVGForExport(element, width, height, currentModule) {
    ...
    for (const element of clone.getElementsByTagName('*')) {
      ...
      const strokeResult = this.processColorAttribute(
        themeManager,
        strokeValue
      );
      if (strokeValue !== strokeResult)
        element.setAttribute('stroke', strokeResult);
