import AbstractGraphLabeler from 'src/graph2/AbstractGraphLabeler';
...
function edgeLabelFormatter(string, allowNull = false) {
  ...
  for (let i = 0; i < length; ++i) {
    ...
    for (let j = 0; j < symbolLength; ++j) {
      if (readSymbol === null) {
        ...
        if (isValidSymbol(symbol)) {
          ...
          if (
            j + 1 < symbolLength &&
            (symbols[j + 1] === READ_SEPARATOR ||
              symbols[j + 1] === SYMBOL_SEPARATOR)
          ) {
            ++j;
          }
        }
    ...
    if (readSymbol === null) {
      ...
      result.add(
        readSymbol + READ_SEPARATOR + popSymbol + POP_SEPARATOR + pushSymbol
      );
    }
  }
