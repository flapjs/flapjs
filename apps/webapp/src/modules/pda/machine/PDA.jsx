import GraphElement from 'src/graph2/element/GraphElement';
import { guid, stringHash } from 'src/util/MathHelper';
...
export class Transition {
  ...
  getHashString() {
    ...
    return (
      this._from.getHashString() +
      ':' +
      symbolString +
      ':' +
      this._to.getHashString()
    );
  }
}
...
class PDA {
  ...
  addState(state) {
    ...
    if (this._states.has(stateID))
      throw new Error("Already added state with id '" + stateID + "'");
  ...
  hasState(state) {
    return this._states.has(state.getStateID());
  }
  ...
  addTransition(from, to, readSymbol, popSymbol, pushSymbol) {
    if (!this.hasState(from))
      throw new Error(
        "Trying to add a transition to unknown state with label '" +
          from.getStateLabel() +
          "'"
      );
    if (!this.hasState(to))
    ...
    if (!readSymbol)
      throw new Error(
        'Cannot add transition for null read symbol - use the empty symbol instead'
      );
    if (!popSymbol)
      throw new Error(
        'Cannot add transition for null pop symbol - use the empty symbol instead'
      );
    if (!pushSymbol)
      throw new Error(
        'Cannot add transition for null push symbol - use the empty symbol instead'
      );
  ...
  _decrSymbolCount(symbol) {
    if (!this._alphabet.has(symbol))
      throw new Error('Unable to find valid transition symbol in alphabet');
  ...
  changeSymbol(symbol, newSymbol) {
    if (symbol === EMPTY_SYMBOL)
      throw new Error('Cannot change the empty symbol');
    if (newSymbol === EMPTY_SYMBOL)
      throw new Error('Cannot change to the empty symbol');
    if (this._stackAlphabet.has(newSymbol))
      throw new Error('Cannot change symbol to another existing symbol');
  ...
  setCustomSymbol(symbol, custom = true) {
    if (symbol === EMPTY_SYMBOL)
      throw new Error('Cannot change the empty symbol as a custom symbol');
    ...
    if (custom) {
      ...
      if (this._customSymbols.has(symbol)) {
        ...
        if (this._alphabet.has(symbol) && this._alphabet.get(symbol) <= 0)
          this._alphabet.delete(symbol);
      }
    }
  }
  ...
  setCustomStackSymbol(symbol, custom = true) {
    ...
    if (custom) {
      ...
      if (this._customStackSymbols.has(symbol)) {
        ...
        //If symbol is unused, delete it
        if (
          this._stackAlphabet.has(symbol) &&
          this._stackAlphabet.get(symbol) <= 0
        )
          this._stackAlphabet.delete(symbol);
      }
    }
  ...
  isUsedStackSymbol(symbol) {
    return (
      this._stackAlphabet.has(symbol) && this._stackAlphabet.get(symbol) > 0
    );
  }
  ...
  setFinalState(state, final = true) {
    ...
    if (final) {
      ...
      if (!this._states.has(state.getStateID()))
        throw new Error(
          "Trying to change final for missing state '" +
            state.getStateLabel() +
            "'"
        );
  ...
  isFinalState(state) {
    return this._finalStates.has(state);
  }
  getFinalStates() {
  ...
  doTransition(state, readSymbol, stack, dst = []) {
    ...
    if (!(state instanceof State))
      throw new Error("Invalid state instance type '" + typeof state + "'");
    if (!this._states.has(state.getStateID()))
      throw new Error(
        "Unable to find source state with id '" + state.getStateID() + "'"
      );
    ...
    for (const key of this._transitions.keys()) {
      if (key.startsWith(fromTransitionKey)) {
        ...
        const validSymbols = [];
        transition.getSymbolsByReadPop(
          readSymbol,
