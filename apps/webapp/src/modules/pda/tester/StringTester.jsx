import Eventable from 'src/util/Eventable';

import { solvePDAByStep } from 'src/modules/pda/machine/PDAUtils';
...
class StringTester {
  ...
  startTest(testString, graphController, machineController) {
    ...
    if (this._tapeContext)
      throw new Error('Unable to start an already running test');
    ...
    this._cachedResult = null;
    this._tapeContext = new TestTapeContext(
      this,
      graphController,
      machineController
    );
  ...
  stopTest() {
    ...
    if (!this._resolve)
      throw new Error('Must call startTest() before anything else');
    if (!this._reject)
      throw new Error('Must call startTest() before anything else');
  ...
  stepForward(graphController, machineController, cacheStep = true) {
    ...
    if (this._cachePath.length <= this._testIndex) {
      ...
      if (this._testIndex <= 0) {
        ...
        for (const relatedStateAndStack of machine.doClosureTransition(
          startState,
          []
        )) {
          cachedStates.push({
      ...
      if (isResult) {
        this._cachedResult = solvePDAByStep(
          machine,
          null,
          cachedStates,
          cachedSymbols
      ...
      for (const cachedState of cachedStates) {
        ...
        if (!node)
          throw new Error("Could not find node '" + cachedState.state + "'");
