import { convertToNFA } from '../REUtils';
import FSA, { EMPTY_SYMBOL } from 'src/modules/fsa2/machine/FSA';
import RE, { EMPTY, CONCAT, UNION, KLEENE, SIGMA, EMPTY_SET } from '../RE';
...
function compareConvertRE(convertedFSA, expectedFSA) {
  ...
  //Same start
  if (
    convertedFSA.getStartState().getStateLabel() !=
    expectedFSA.getStartState().getStateLabel()
  ) {
    sameStart = false;
  }
  //Same final state (there should only be one)
  if (
    convertedFSA.getFinalStates().length != expectedFSA.getFinalStates().length
  ) {
    sameFinal = false;
  } else {
    let convertedFinals = [];
    ...
    if (convertedFinals[0] != expectedFinals[0]) sameFinal = false;
  }
  //Same transitions
  if (
    convertedFSA.getTransitions().length != expectedFSA.getTransitions().length
  ) {
    ...
    for (const transition of convertedFSA.getTransitions()) {
      for (const symbol of transition.getSymbols()) {
        convertedTransitions.push([
          transition._from.getStateLabel(),
          transition._to.getStateLabel(),
          symbol,
        ]);
      }
    }
