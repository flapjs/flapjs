...
import { EMPTY, CONCAT, UNION, KLEENE, SIGMA, EMPTY_SET, PLUS } from '../RE';
...
function ASTtoNFA(astNode, re) {
  ...
  switch (astNode.getSymbol()) {
    ...
    case CONCAT:
      return concat(
        ASTtoNFA(astNode._children[0], re),
        ASTtoNFA(astNode._children[1], re)
      );
    case UNION:
      return or(
        ASTtoNFA(astNode._children[0], re),
    ...
    default:
      throw new Error(
        "You've got a weird node in the AST tree with symbol " +
          astNode.getSymbol()
      );
  }
}
...
function concat(a, b) {
  ...
  for (const transition of aTransitions) {
    ...
    if (newFromState === null || newToState === null)
      throw new Error('Unable to find state endpoints for transition');
